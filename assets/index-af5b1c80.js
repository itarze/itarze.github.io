...
`;styled.div`
  ...
  padding: 1.5rem 3rem;
  gap: 2rem;
`;const printId=e=>`0x${e.toString(16).padStart(4,"0").toUpperCase()}`,printBytes=e=>e.join(" "),AppErrors=({})=>{const e=useAppSelector(getAppErrors),t=useDispatch();return jsxs(Fragment,{children:[jsxs(IconButtonGroupContainer,{style:{margin:"10px 15px"},children:[jsxs(IconButtonContainer,{onClick:()=>t(clearAppErrors()),children:[jsx(FontAwesomeIcon,{size:"sm",color:"var(--color_label)",icon:faCancel}),jsx(IconButtonTooltip,{children:"Clear"})]}),jsxs(IconButtonContainer,{onClick:()=>saveAppErrors(e),children:[jsx(FontAwesomeIcon,{size:"sm",color:"var(--color_label)",icon:faDownload}),jsx(IconButtonTooltip,{children:"Download"})]})]}),e.map(([o,{name:n,message:r,cause:s}])=>jsxs(Container$1,{children:[o,jsxs("ul",{children:[jsxs("li",{children:["Name: ",n]}),jsxs("li",{children:["Message: ",r]}),jsxs("li",{children:["Cause: ",`${s}`]})]})]},o))]})},KeyboardAPIErrors=({})=>{const e=useAppSelector(getKeyboardAPIErrors),t=useDispatch();return jsxs(Fragment,{children:[jsxs(IconButtonGroupContainer,{style:{margin:"10px 15px"},children:[jsxs(IconButtonContainer,{onClick:()=>t(clearKeyboardAPIErrors()),children:[jsx(FontAwesomeIcon,{size:"sm",color:"var(--color_label)",icon:faCancel}),jsx(IconButtonTooltip,{children:"Clear"})]}),jsxs(IconButtonContainer,{onClick:()=>saveKeyboardAPIErrors(e),children:[jsx(FontAwesomeIcon,{size:"sm",color:"var(--color_label)",icon:faDownload}),jsx(IconButtonTooltip,{children:"Download"})]})]}),e.map(({timestamp:o,commandName:n,commandBytes:r,responseBytes:s,device:a})=>jsxs(Container$1,{children:[o,jsxs("ul",{children:[jsxs("li",{children:["Vid: ",printId(a.vendorId)]}),jsxs("li",{children:["Pid: ",printId(a.productId)]}),jsxs("li",{children:["Command name: ",n]}),jsxs("li",{children:["Command: ",printBytes(r)]}),jsxs("li",{children:["Response: ",printBytes(s)]})]})]},o))]})},saveKeyboardAPIErrors=async e=>{try{const t=await window.showSaveFilePicker({suggestedName:"VIA-keyboard-API-errors.csv",types:[{accept:{"text/csv":[".csv"]},description:"CSV file"}]}),o=["timestamp, vid, pid, commandName, command, response"],n=e.map(({timestamp:i,commandName:c,commandBytes:l,responseBytes:d,device:u})=>`${i}, ${printId(u.vendorId)}, ${printId(u.productId)}, ${c}, ${printBytes(l)}, ${printBytes(d)}`),r=o.concat(...n).join(`
`),s=new Blob([r],{type:"text/csv"}),a=await t.createWritable();await a.write(s),await a.close()}catch{console.log("User cancelled save errors request")}},saveAppErrors=async e=>{try{const t=await window.showSaveFilePicker({suggestedName:"VIA-app-errors.csv"}),o=["timestamp, name, message, cause"],n=e.map(([i,{name:c,message:l,cause:d}])=>`${i}, ${c}, ${l}, ${d}`),r=o.concat(...n).join(`
`),s=new Blob([r],{type:"text/csv"}),a=await t.createWritable();await a.write(s),await a.close()}catch{console.log("User cancelled save errors request")}},IconButtonGroupContainer=styled.div`
  border-radius: 2px;
  border: 1px solid var(--border_color_icon);
  display: inline-flex;
  > button:last-child {
