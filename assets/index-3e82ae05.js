...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,a]=reactExports.useState(!0),[i,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,C]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:_,recordDelaysEnabled:h}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[K,y]=useKeycodeRecorder(i,h),g=reactExports.useRef(null),m=reactExports.useCallback(async L=>{c(L),L?(await navigator.keyboard.lock(),y([]),a(!1),d(!0)):(navigator.keyboard.unlock(),_&&y(optimizeKeycodeSequence(K)),d(!1))},[K,c]),S=reactExports.useCallback(()=>{o(""),a(!0),d(!1)},[y,o]),f=reactExports.useCallback(()=>{n(),y([]),a(!0),d(!1)},[n]);reactExports.useEffect(()=>{a(!0),d(!1),y([])},[e]);const x=()=>[...s?e??[]:K],A=reactExports.useMemo(()=>{let L,T=x();return s||!l||!_?L=T:L=optimizeKeycodeSequence(T),L},[K,s,_,l,e]);reactExports.useEffect(()=>{A&&t(sequenceToExpression(A))},[A]);const w=reactExports.useCallback(()=>{s&&a(!1)},[s]),O=reactExports.useCallback(L=>{const T=x();T.splice(L,1),y(cleanKeycodeSequence(T)),w()},[A,e,K,s]),v=reactExports.useCallback((L,T)=>{const R=x();R.splice(L,1,[RawKeycodeSequenceAction.Delay,T]),y(cleanKeycodeSequence(R)),w()},[A,e,K,s]),B=reactExports.useMemo(()=>componentJoin(A.map(([L,T],R)=>{const I=getSequenceItemComponent(L);return jsx(Deletable,{index:R,deleteItem:O,disabled:i,children:RawKeycodeSequenceAction.Delay!==L?jsx(I,{children:L===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(T).split(" ").map((P,F)=>jsx("span",{children:P},F)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(T)?T.map(P=>getSequenceLabel(KeycodeMap[P])??P).join(" + "):getSequenceLabel(KeycodeMap[T])}):jsx(WaitInput,{index:R,value:Number(T),updateValue:v})},`${R}-${L}`)}),jsx(SequenceLabelSeparator,{})),[A]);reactExports.useEffect(()=>{const L=()=>{C(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",L),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",L)}},[C]);const b=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:B.length?B:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:_,recordDelays:h,isRecording:i,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!_}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!h}))},toggleFullscreen:b,undoChanges:f,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
...
`,ColumnDiv=styled.div`
  display: flex;
  flex-direction: column;
`;class EncoderModeToggle extends React.Component{constructor(){super(...arguments);M(this,"handleInputChange",o=>{const{enabledModes:n,onChange:r}=this.props,{target:{checked:s,name:a}}=o,i=1<<MODES[a],c=s?n|i:n&~i;r(c)});M(this,"isChecked",o=>(1<<o&this.props.enabledModes)>0)}render(){return jsxs(CenteredColumnDiv,{children:[jsx("h3",{children:"Enabled Encoder Modes:"}),jsx("p",{children:"Only the selected encoder modes will be available on the keyboard"}),jsx(ColumnDiv,{children:Object.entries(MODES).map(([o,n])=>jsxs("label",{htmlFor:MODE_LABELS[o],children:[jsx("input",{name:o,id:MODE_LABELS[o],type:"checkbox",checked:this.isChecked(n),onChange:this.handleInputChange},n),MODE_LABELS[o]]},n))})]})}}const NormalInput=styled.input`
  border: none;
  border-bottom: 1px solid var(--bg_control);
  color: var(--color_accent);
  background: var(--bg_menu);
