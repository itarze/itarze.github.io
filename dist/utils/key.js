...
export const categoriesForKeycodeModule = (keycodeModule) => ({
...
export const getKeycodesForKeyboard = (definition) => {
  let includeList = [];
  if ("lighting" in definition) {
    const {keycodes} = getLightingDefinition(definition.lighting);
    includeList = categoriesForKeycodeModule("default").concat(keycodes === KeycodeType.None ? [] : keycodes === KeycodeType.QMK ? categoriesForKeycodeModule(BuiltInKeycodeModule.QMKLighting) : categoriesForKeycodeModule(BuiltInKeycodeModule.WTLighting));
  } else {
    const {keycodes} = definition;
    includeList = keycodes.flatMap(categoriesForKeycodeModule);
  }
  return getKeycodes().flatMap((keycodeMenu) => includeList.includes(keycodeMenu.label) ? keycodeMenu.keycodes : []).sort((a, b) => {
    if (a.code <= b.code) {
      return -1;
    } else {
      return 1;
